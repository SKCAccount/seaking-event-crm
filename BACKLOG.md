...
10. **Scanner observability/alerting:** a daily **digest email** (Resend; set `RESEND_API_KEY` + `DIGEST_TO`) was added — it sends after every run (including a "nothing new today" note), doubling as a health heartbeat. Key is now set and a test digest delivered to skcaccounts@seakingcapital.com; PENDING `seakingcapital.com` domain verification in Resend to deliver to derek@seakingcapital.com (Resend only emails the account address until the domain is verified). Still no scan-state table; could add an `inbox_scan_state` row or a louder alert if a run errors or finds nothing for N days.
